...
\runningheader{Assignment 2}{Detection and Estimation Theory}{Dwaipayan Haldar}
...
\questionheader{Problem 6 (Continued)}

\begin{solution}
\textbf{(b)} Let $y \sim \mathcal{N}(s, R(\theta))$ where $s$ is a known vector and $R(\theta)$ is the covariance matrix that depends on $\theta$.

The pdf is:
\[
f_{\theta}(y) = \frac{1}{(2\pi)^{n/2}|R(\theta)|^{1/2}} \exp\left(-\frac{1}{2}(y-s)^T R^{-1}(\theta)(y-s)\right)
\]

The log-likelihood is:
\[
\ln f_{\theta}(y) = -\frac{n}{2}\ln(2\pi) - \frac{1}{2}\ln|R(\theta)| - \frac{1}{2}(y-s)^T R^{-1}(\theta)(y-s)
\]

Using the matrix calculus identities:
\begin{itemize}
    \item $\frac{\partial}{\partial \theta_j}\ln|R| = \text{Trace}\left(R^{-1}\frac{\partial R}{\partial \theta_j}\right)$
    \item $\frac{\partial R^{-1}}{\partial \theta_j} = -R^{-1}\frac{\partial R}{\partial \theta_j}R^{-1}$
\end{itemize}

Taking the derivative with respect to $\theta_j$:
\[
\frac{\partial \ln f_{\theta}(y)}{\partial \theta_j} = -\frac{1}{2}\text{Trace}\left(R^{-1}\frac{\partial R}{\partial \theta_j}\right) + \frac{1}{2}(y-s)^T R^{-1}\frac{\partial R}{\partial \theta_j}R^{-1}(y-s)
\]

Let $z = y - s \sim \mathcal{N}(0, R(\theta))$ and define $A_j = R^{-1}\frac{\partial R}{\partial \theta_j}$. Then:
\[
\frac{\partial \ln f_{\theta}(y)}{\partial \theta_j} = \frac{1}{2}\left[z^T A_j R^{-1} z - \text{Trace}(A_j)\right] = \frac{1}{2}\text{Trace}\left(A_j(R^{-1}zz^T - I)\right)
\]

Using the Fisher information formula $[I_y(\theta)]_{jk} = E\left[\frac{\partial \ln f}{\partial \theta_j}\frac{\partial \ln f}{\partial \theta_k}\right]$ and the property for Gaussian random vectors that:
\[
E[\text{Trace}(A_j(R^{-1}zz^T - I))\text{Trace}(A_k(R^{-1}zz^T - I))] = 2\text{Trace}(A_j A_k)
\]

We get:
\[
\boxed{[I_y(\theta)]_{jk} = \frac{1}{2}\text{Trace}\left(R^{-1}(\theta)\frac{\partial R(\theta)}{\partial \theta_j}R^{-1}(\theta)\frac{\partial R(\theta)}{\partial \theta_k}\right)}
\]

\vspace{1em}
\textbf{(c)} Let $y \sim \mathcal{N}(s(\theta), R(\theta))$ where both mean and covariance depend on $\theta$.

The log-likelihood is:
\[
\ln f_{\theta}(y) = -\frac{n}{2}\ln(2\pi) - \frac{1}{2}\ln|R(\theta)| - \frac{1}{2}(y-s(\theta))^T R^{-1}(\theta)(y-s(\theta))
\]

Taking the derivative:
\[
\frac{\partial \ln f}{\partial \theta_j} = \underbrace{\left(\frac{\partial s}{\partial \theta_j}\right)^T R^{-1}(y-s)}_{\text{from mean}} + \underbrace{\left[-\frac{1}{2}\text{Trace}\left(R^{-1}\frac{\partial R}{\partial \theta_j}\right) + \frac{1}{2}(y-s)^T R^{-1}\frac{\partial R}{\partial \theta_j}R^{-1}(y-s)\right]}_{\text{from covariance}}
\]

When computing the Fisher information, the cross terms between the mean-dependent and covariance-dependent parts vanish because:
\begin{itemize}
    \item The mean-dependent term is linear in $(y-s)$, which has zero mean
    \item The covariance-dependent terms are either constant or quadratic in $(y-s)$
    \item $E[(y-s)] = 0$ implies the cross-product expectation vanishes
\end{itemize}

Therefore, the Fisher information matrix is \textbf{additive}:
\[
\boxed{[I_y(\theta)]_{jk} = \left[\frac{\partial s(\theta)}{\partial \theta_j}\right]^T R^{-1} \left[\frac{\partial s(\theta)}{\partial \theta_k}\right] + \frac{1}{2}\text{Trace}\left(R^{-1}\frac{\partial R}{\partial \theta_j}R^{-1}\frac{\partial R}{\partial \theta_k}\right)}
\]
This is the sum of the Fisher information matrices from parts (a) and (b).
\end{solution}

\questionheader{Problem 7 (Continued)}

\begin{solution}
\textbf{(b)} From part (a), we have $y_k \sim \mathcal{N}(0, \theta s_k^2 + 1)$ independently.

The log-likelihood is:
\[
\ln p_{\theta}(y) = \sum_{k=1}^{N} \left[-\frac{1}{2}\ln(2\pi) - \frac{1}{2}\ln(\theta s_k^2 + 1) - \frac{y_k^2}{2(\theta s_k^2 + 1)}\right]
\]

Taking derivatives:
\[
\frac{\partial \ln p_{\theta}(y)}{\partial \theta} = \sum_{k=1}^{N} \left[-\frac{s_k^2}{2(\theta s_k^2 + 1)} + \frac{s_k^2 y_k^2}{2(\theta s_k^2 + 1)^2}\right]
\]

\[
\frac{\partial^2 \ln p_{\theta}(y)}{\partial \theta^2} = \sum_{k=1}^{N} \left[\frac{s_k^4}{2(\theta s_k^2 + 1)^2} - \frac{s_k^4 y_k^2}{(\theta s_k^2 + 1)^3}\right]
\]

The Fisher Information is:
\[
I_y(\theta) = -E\left[\frac{\partial^2 \ln p_{\theta}(y)}{\partial \theta^2}\right]
\]

Since $E[y_k^2] = \theta s_k^2 + 1$:
\begin{align*}
I_y(\theta) &= -\sum_{k=1}^{N} \left[\frac{s_k^4}{2(\theta s_k^2 + 1)^2} - \frac{s_k^4(\theta s_k^2 + 1)}{(\theta s_k^2 + 1)^3}\right] \\
&= -\sum_{k=1}^{N} \left[\frac{s_k^4}{2(\theta s_k^2 + 1)^2} - \frac{s_k^4}{(\theta s_k^2 + 1)^2}\right] \\
&= \sum_{k=1}^{N} \frac{s_k^4}{2(\theta s_k^2 + 1)^2}
\end{align*}

Therefore, the \textbf{Cramer-Rao Bound} is:
\[
\boxed{\text{CRB}(\theta) = \frac{1}{I_y(\theta)} = \frac{1}{\sum_{k=1}^{N} \frac{s_k^4}{2(\theta s_k^2 + 1)^2}}}
\]

\vspace{1em}
\textbf{(c)} When $s_i \in \{+1, -1\}$, we have $s_i^2 = 1$ for all $i$.

So $y_k \sim \mathcal{N}(0, \theta + 1)$ independently for all $k$.

The log-likelihood becomes:
\[
\ln p_{\theta}(y) = -\frac{N}{2}\ln(2\pi) - \frac{N}{2}\ln(\theta + 1) - \frac{1}{2(\theta + 1)}\sum_{k=1}^{N} y_k^2
\]

Setting the derivative to zero:
\[
\frac{\partial \ln p_{\theta}(y)}{\partial \theta} = -\frac{N}{2(\theta + 1)} + \frac{\sum_{k=1}^{N} y_k^2}{2(\theta + 1)^2} = 0
\]

Solving:
\[
\frac{N}{\theta + 1} = \frac{\sum_{k=1}^{N} y_k^2}{(\theta + 1)^2}
\]
\[
\Rightarrow \theta + 1 = \frac{1}{N}\sum_{k=1}^{N} y_k^2
\]

Since we require $\theta \geq 0$, the \textbf{ML estimate} is:
\[
\boxed{\hat{\theta}_{ML} = \max\left(0, \frac{1}{N}\sum_{k=1}^{N} y_k^2 - 1\right)}
\]

For typical scenarios where the unconstrained estimate is positive:
\[
\hat{\theta}_{ML} = \frac{1}{N}\sum_{k=1}^{N} y_k^2 - 1
\]

\vspace{1em}
\textbf{(d)} Let $\hat{\theta} = \frac{1}{N}\sum_{k=1}^{N} y_k^2 - 1$ (ignoring the boundary constraint for analysis).

\textbf{Bias:}
\[
E[\hat{\theta}] = \frac{1}{N}\sum_{k=1}^{N} E[y_k^2] - 1 = \frac{1}{N} \cdot N \cdot (\theta + 1) - 1 = \theta
\]

$\Rightarrow$ \textbf{Bias} $= E[\hat{\theta}] - \theta = 0$ (unbiased estimator)

\textbf{Variance:}

For $y_k \sim \mathcal{N}(0, \sigma^2)$ where $\sigma^2 = \theta + 1$:
\[
\text{Var}(y_k^2) = E[y_k^4] - (E[y_k^2])^2 = 3\sigma^4 - \sigma^4 = 2\sigma^4 = 2(\theta + 1)^2
\]

Since the $y_k$ are independent:
\[
\text{Var}(\hat{\theta}) = \text{Var}\left(\frac{1}{N}\sum_{k=1}^{N} y_k^2\right) = \frac{1}{N^2}\sum_{k=1}^{N} \text{Var}(y_k^2) = \frac{N \cdot 2(\theta + 1)^2}{N^2} = \frac{2(\theta + 1)^2}{N}
\]

\textbf{Comparison with CRB:}

From part (b) with $s_k^2 = 1$:
\[
I_y(\theta) = \sum_{k=1}^{N} \frac{1}{2(\theta + 1)^2} = \frac{N}{2(\theta + 1)^2}
\]

\[
\text{CRB} = \frac{1}{I_y(\theta)} = \frac{2(\theta + 1)^2}{N}
\]

\textbf{Conclusion:}
\[
\boxed{\text{Var}(\hat{\theta}_{ML}) = \frac{2(\theta + 1)^2}{N} = \text{CRB}}
\]

The ML estimator achieves the Cramer-Rao Bound with equality, so it is an \textbf{efficient estimator}. Combined with being unbiased, the ML estimator $\hat{\theta}_{ML} = \frac{1}{N}\sum_{k=1}^{N} y_k^2 - 1$ is the \textbf{MVU estimator} for the case when $s_i \in \{+1, -1\}$.
\end{solution}
